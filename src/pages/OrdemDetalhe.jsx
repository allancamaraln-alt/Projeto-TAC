...
import { formatOS, formatBRL, formatDate, formatTime, formatGarantia, resumoPagamento } from '../lib/format'
...
export default function OrdemDetalhe() {
  ...
  const [visualizandoRecibo, setVisualizandoRecibo] = useState(false)
  ...
  // Trava o scroll da página por trás enquanto o modal de conclusão está
  // aberto — sem isso, ao rolar dentro do modal a página de fundo também
  // se mexe (efeito "solto"), especialmente perceptível no iOS.
  useEffect(() => {
    if (mostrarModalConclusao) document.body.style.overflow = 'hidden'
  ...
  async function visualizarPDF(gerarFn, nomeArquivo, setLoadingState) {
    // Precisa abrir a aba de forma síncrona, ainda dentro do clique do
    // usuário — no Safari/iOS um window.open() feito depois de um await
    // perde o "user gesture" e é bloqueado silenciosamente como popup.
    const isNative = Capacitor.isNativePlatform()
    const preview = isNative ? null : window.open('', '_blank')
    if (!isNative && !preview) {
      toast('Permita pop-ups no navegador para visualizar o PDF', 'error')
      return
    }
    ...
    try {
      ...
      if (isNative) {
        ...
        const base64 = doc.output('datauristring').split(',')[1]
        const { uri } = await Filesystem.writeFile({ path: nomeArquivo, data: base64, directory: Directory.Cache })
        await Share.share({ title: nomeArquivo.replace('.pdf', ''), url: uri, dialogTitle: 'Visualizar' })
        return
      }

      preview.location.href = doc.output('bloburl')
    } catch {
      preview?.close()
  ...
  function visualizarOrcamento() {
    const nome = `Orcamento-${formatOS(os.numero)}-${os.clientes?.nome?.split(' ')[0]}.pdf`
    return visualizarPDF(async () => {
      const { gerarOrcamentoPDF } = await import('../lib/pdf')
      return gerarOrcamentoPDF({ cliente: os.clientes, ordem: os, tecnico: profile })
    }, nome, setVisualizando)
  }
  ...
  function visualizarRecibo() {
    ...
    return visualizarPDF(async () => {
      ...
      return gerarReciboPDF({ cliente: os.clientes, ordem: os, tecnico: profile, fotos })
    }, nome, setVisualizandoRecibo)
  }
  ...
  return (
    ...
    <div className="page-container">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        {/* Botão Enviar Orçamento + Visualizar PDF */}
        <div className="flex items-stretch gap-2">
          <button
            onClick={enviarWhatsApp}
            disabled={enviando}
            className="flex-1 bg-green-500 text-white font-bold py-4 px-4 rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-70"
          ...
          <button
            ...
            title="Visualizar OS em PDF"
            className="w-14 flex-shrink-0 bg-white border border-gray-200 text-gray-600 rounded-2xl flex items-center justify-center active:scale-95 transition-all disabled:opacity-70"
          >
            {visualizando ? (
        ...
        {/* Botão Enviar Recibo + Visualizar PDF (apenas quando concluído) */}
        {os.status === 'concluido' && (
          <div className="flex items-stretch gap-2">
            <button
              onClick={enviarRecibo}
              disabled={enviandoRecibo}
              className="flex-1 text-white font-bold py-4 px-4 rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-70"
            ...
            <button
              onClick={visualizarRecibo}
              disabled={visualizandoRecibo}
              title="Visualizar Recibo em PDF"
              className="w-14 flex-shrink-0 bg-white border border-gray-200 text-gray-600 rounded-2xl flex items-center justify-center active:scale-95 transition-all disabled:opacity-70"
            >
              {visualizandoRecibo ? (
                <span className="w-5 h-5 border-2 border-gray-300 border-t-gray-500 rounded-full animate-spin" />
      ...
      >
        <div className="bg-white w-full rounded-t-3xl px-5 pt-6 pb-24 space-y-5 max-h-[90vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="space-y-2">
              {conclusaoForm.pagamentos.map((p, i) => (
                <div key={i} className="flex gap-2">
                  <select
                    className="input-field flex-1"
                    value={p.forma}
                    onChange={e => setConclusaoForm(prev => ({
                      ...prev,
                      pagamentos: prev.pagamentos.map((x, xi) => xi === i ? { ...x, forma: e.target.value } : x),
                    }))}
                  >
                  ...
                  <input
                    ...
                    onChange={e => setConclusaoForm(prev => ({
                      ...prev,
                      pagamentos: prev.pagamentos.map((x, xi) => xi === i ? { ...x, valor: e.target.value } : x),
                    }))}
                  />
                  {conclusaoForm.pagamentos.length > 1 && (
                    <button
                      type="button"
                      onClick={() => setConclusaoForm(prev => ({ ...prev, pagamentos: prev.pagamentos.filter((_, xi) => xi !== i) }))}
                      className="w-9 h-9 flex-shrink-0 flex items-center justify-center rounded-full text-gray-400 active:bg-gray-100"
                    >
            ...
            <button
              type="button"
              onClick={() => setConclusaoForm(prev => ({ ...prev, pagamentos: [...prev.pagamentos, { forma: '', valor: '' }] }))}
              className="text-sm font-semibold ac-text mt-2"
            >
            ...
            <div className="mt-3 p-3 rounded-xl bg-gray-50 space-y-1">
              ...
              {saldoDevedorForm > 0.004 && (
                <div className="flex justify-between text-sm pt-1.5 mt-1 border-t border-gray-200">
                  ...
                  <span className="text-amber-600 font-semibold">{formatBRL(saldoDevedorForm)}</span>
                </div>
              )}
            </div>
            ...
            {saldoDevedorForm > 0.004 && (
              <div className="mt-3">
                <p className="text-sm font-semibold text-gray-700 mb-1.5">
                  Previsão de pagamento do restante <span className="font-normal text-gray-400">(opcional)</span>
                </p>
                <input
                  type="date"
                  className="input-field"
                  value={conclusaoForm.data_pagamento_pendente}
                  onChange={e => setConclusaoForm(prev => ({ ...prev, data_pagamento_pendente: e.target.value }))}
                />
              </div>
            )}
          </div>
